...
def yes_no_question():
    ...
    match options:
        case 1: 
            ...
            print( 'Result: ', render_questions(card_list= MEANING_TYPE, card_type= sub_name) )
        case 2: 
...
def draw_tarot_cards():
    ...
    card_list = []
    temp_list = [] # tạo 1 list giả để kiểm tra 2 thẻ giống nhau trong cùng 1 lá bài

    for nb in range(number_of_card):
        card = tarot.get_info_card()
        temp_card = card.split(' - ')[0]
        if (temp_card in temp_list) == False: 
